...
public class TC_ECOM_Reg_016 extends BaseTest {

    @Test
    public void verifyMrsRadioButtonInSignupPage() throws InterruptedException {

        // Create Extent Report entry
        test = extent.createTest("TC_ECOM_Reg_016 - Verify Mrs Radio Button in Signup Page");
        ...
        test.info("Entered user details: Name and Unique Email");
        ...
        // Step 5: Verify navigation to 'Enter Account Information'
        ...
        Assert.assertTrue(enterAccountInfo.isDisplayed(), "Failed to navigate to Enter Account Information page");
        test.pass("User successfully navigated to Enter Account Information page.");

        // Step 6: Click on 'Mrs' radio button
        WebElement mrsRadioButton = wait.until(ExpectedConditions.elementToBeClickable(By.id("id_gender2")));
        ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", mrsRadioButton);
        Thread.sleep(500); // Optional: to visually see the click
        mrsRadioButton.click();
        Assert.assertTrue(mrsRadioButton.isSelected(), "Mrs radio button is not selected");
        test.pass("PASS - Mrs radio button is successfully clicked.");
    }
}
