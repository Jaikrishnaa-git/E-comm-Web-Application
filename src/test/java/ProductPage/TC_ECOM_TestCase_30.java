...
public class TC_ECOM_TestCase_30 extends BaseTest {

    @Test
    public void verifyEmailNoInputInProductPage() throws InterruptedException {

        // Create Extent Report entry
        test = extent.createTest("TC_ECOM_TestCase_30 - Verify Email with no input in Product Page");
        ...
        // Step 4: Locate Email input field and clear it
        WebElement emailInput = wait.until(ExpectedConditions.visibilityOfElementLocated(
            By.xpath("//input[@id='email']")  // Adjust ID if different
        ));
        emailInput.clear();
        test.info("Ensured Email field is empty.");

        // Step 5: Try to submit the form without entering email
        WebElement submitBtn = driver.findElement(By.xpath("//button[@type='submit']"));
        submitBtn.click();
        test.info("Clicked submit without entering an email.");
        ...
        WebElement emailError = wait.until(ExpectedConditions.visibilityOfElementLocated(
            By.xpath("//input[@id='email'][@required]")  // HTML5 required validation
        ));

        Assert.assertTrue(emailError.isDisplayed(), "Form submission should be prevented without email!");

        test.pass("Form submission is not allowed without entering email, as expected.");
    }
}
