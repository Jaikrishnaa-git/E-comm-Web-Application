...
public class TC_ECOM_TestCase_31 extends BaseTest {

    @Test
    public void verifyAddReviewInputInProductPage() throws InterruptedException {

        // Create Extent Report entry
        test = extent.createTest("TC_ECOM_TestCase_31 - Verify Add Review input in Product Page");
        ...
        // Step 4: Locate Add Review input field
        WebElement reviewInput = wait.until(ExpectedConditions.visibilityOfElementLocated(
            By.xpath("//textarea[@id='review']")  // Adjust ID if different
        ));

        // Step 5: Enter a review
        reviewInput.clear();
        reviewInput.sendKeys("This is a great Stylish Dress! Highly recommended.");
        test.info("Entered input in Add Review field.");

        // Validate that the review text is entered
        String enteredReview = reviewInput.getAttribute("value");
        Assert.assertEquals(enteredReview, "This is a great Stylish Dress! Highly recommended.", "Review input value mismatch!");

        test.pass("Successfully entered input in Add Review section on the product page.");
    }
}
