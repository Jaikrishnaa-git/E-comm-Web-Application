...
public class TC_ECOM_TestCase_34 extends BaseTest {

    @Test
    public void verifyProductAvailabilityInProductPage() throws InterruptedException {

        test = extent.createTest("TC_ECOM_TestCase_34 - Verify Product Availability in Product Page");
        ...
        // Step 6: Locate the Availability element
        WebElement availabilityElement = productInfo.findElement(
            By.xpath(".//b[text()='Availability:']/parent::p")
        );
        ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", availabilityElement);
        Thread.sleep(1000);

        // Step 7: Verify that availability is displayed
        String availabilityText = availabilityElement.getText();
        Assert.assertTrue(availabilityText.contains("Availability"), "Product availability is not displayed!");
        test.info("Product availability is visible: " + availabilityText);

        test.pass("Successfully verified that product availability is visible on the product page.");
    }
}
