...
@Test(dataProvider ="homePageData")
  public void verifySubsscription(String email,String dummy) throws IOException, InterruptedException {
	  ...
	  {
		  test.pass("Subscription with invalid mail not allowed");
	  }
	  else
	  {
		  test.fail("Subscribed with invalid email").addScreenCaptureFromPath(ScreenshotUtilities.capturescreen(driver,"TC_ECOM_subHome_001"));
		  Assert.fail("Subscribed with invalid Email");
	  }
  ...
  public Object[][] homePageData()
  {
	  try {
		  ...
		  Object[][] data = ExcelUtilities.getdata(projectpath + "\\src\\test\\resources\\Testdata\\Subscription data.xlsx", "invalid email");
